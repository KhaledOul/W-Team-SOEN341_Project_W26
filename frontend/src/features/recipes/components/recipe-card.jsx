...
export default function RecipeCard({ recipe, onEdit, onDelete }) {
  ...
  const firstStep = (stepsText || '').split('\n').filter((item) => item.trim())[0] || '';
    ...
    <div className="recipe-card">
      ...
      <div className="recipe-meta">
        <div className="meta-item">
          ...
          <span className="meta-value">{recipe.author || recipe.authorName || '-'}</span>
        </div>
        ...
        <div className="meta-item">
          ...
          <span className="meta-value">${Number(recipe.cost || 0).toFixed(2)}</span>
        </div>
      </div>
      ...
      <div className="recipe-section">
        <h4>Steps</h4>
        {firstStep ? (
          <div className="step-truncated">1. {firstStep.trim()}</div>
        ) : (
          <div className="step-truncated step-empty">No steps added</div>
        )}
      </div>
